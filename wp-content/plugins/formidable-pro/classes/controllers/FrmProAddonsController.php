...
class FrmProAddonsController extends FrmAddonsController {
	...
	public static function show_conditional_action_button( $atts ) {
		...
		if ( $is_addons_page ) {
			self::render_button( esc_html__( 'Deactivate', 'formidable' ), $plugin, $class . ' frm-deactivate-addon frm-button-red' );
		}

		if ( $is_addons_page || $is_installed ) {
			self::render_button( esc_html__( 'Activate', 'formidable' ), $plugin, $class . ' frm-activate-addon' . $additional_class );
		}

		if ( $is_addons_page ) {
			self::render_button( esc_html__( 'Uninstall', 'formidable-pro' ), $plugin, $class . ' frm-uninstall-addon frm-button-red frm-mx-xs' . $additional_class );
		}
		...
		if ( ! empty( $addon['url'] ) ) {
			if ( $is_addons_page || ! $is_installed ) {
				...
				self::render_button( esc_html__( 'Install', 'formidable' ), $addon['url'], $class . ' frm-install-addon' . $additional_class );
			}
			...
			self::render_button( esc_html__( 'Renew Now', 'formidable' ), $upgrade_url, $class . ' install-now button-secondary frm-button-secondary', $upgrade_url, '_blank' );
		} else {
			self::addon_upgrade_link( $addon, $upgrade_link );
		}
	}
	...
	private static function render_button( $text, $rel, $class, $href = '#', $target = '_self' ) {
		$attributes = array(
			'href'   => esc_url( $href ),
			'rel'    => esc_attr( $rel ),
			'class'  => 'button-primary frm-button-primary frm-addon-button ' . esc_attr( $class ),
